...
import { BOSS_DATA, BOSS_DATA_MAP, getBossForStage, isBossStage, getBossDialogue } from './bossData';
  ...
  const evaluateGame = async () => {
      ...
      setGameState(prev => {
          ...
          if (prev.isBossRound && prev.currentBossId && netWin > 0) {
              const boss = prev.currentBossId ? BOSS_DATA_MAP.get(prev.currentBossId) : undefined;
              if (boss) {
    ...
    <GameContainer ref={containerRef} className="h-screen w-full">
      ...
      {gameState.isBossRound && gameState.currentBossId && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[60] w-full max-w-2xl px-4">
          <BossBattlePhases
            boss={gameState.currentBossId ? BOSS_DATA_MAP.get(gameState.currentBossId) || BOSS_DATA[0] : BOSS_DATA[0]}
