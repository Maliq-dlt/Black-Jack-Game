...
// ⚡ Bolt: Precomputed map for O(1) combo lookups, avoiding O(N) array scans during high-frequency scoring calculations
export const COMBO_MAP = new Map(COMBO_BONUSES.map(b => [b.type, b]));
...
export function calculateComboBonus(combos: ComboType[]): { mult: number; gold: number } {
  ...
  combos.forEach(combo => {
    const bonus = COMBO_MAP.get(combo);
    if (bonus) {
