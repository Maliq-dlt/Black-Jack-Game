...
function hasPair(cards: Card[]): boolean {
  ...
  // O(N^2) loop is faster for N < 20 by avoiding array allocation/closure overhead
  for (let i = 0; i < len; i++) {
...
function isSequential(cards: Card[]): boolean {
  ...
  // Pre-allocate array to avoid reallocation
  const values = new Array(len);
  for (let i = 0; i < len; i++) {
    values[i] = getCardValue(cards[i]);
  }

  // Manual insertion sort for small arrays (faster than Array.sort)
  for (let i = 1; i < len; i++) {
