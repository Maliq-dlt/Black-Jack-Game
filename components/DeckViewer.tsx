...
const DeckViewer: React.FC<DeckViewerProps> = ({ isOpen, onClose, deck, removedRanks }) => {
  ...
  // Pre-calculate stats from the already computed `cardCounts` object (O(1) lookups)
  // instead of running multiple redundant deck.filter().length passes (O(N) operations) on every render.
  const highCardsCount = ['10', 'J', 'Q', 'K', 'A'].reduce((sum, rank) => sum + (cardCounts[rank]?.count || 0), 0);
  const acesCount = cardCounts[Rank.Ace]?.count || 0;
  const lowCardsCount = ['2', '3', '4', '5', '6'].reduce((sum, rank) => sum + (cardCounts[rank]?.count || 0), 0);
